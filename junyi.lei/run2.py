...
if __name__ == "__main__":
    ...
    while True:
        ...
        # 累加时间
        target_switch_timer += timestep
        total_sim_timer += timestep  # [修改 3] 累加总仿真时间
        
        # [修改 3]：如果总仿真时间到达40s，自动关闭并退出
        if total_sim_timer >= 40.0:
            print(f"INFO: 仿真已运行 40 秒，自动关闭。")
            p.disconnect()
            plt.close()
            break

        # 满足10s时执行自动切换
        if target_switch_timer >= 10.0:
            sim.current_target = (sim.current_target + 1) % len(sim.targets)
            sim.tello_controller.reset()
            sim.display_target()
            target_switch_timer = 0.0  # 切换后重置计时器
        ...
        if ord("r") in keys and keys[ord("r")] & p.KEY_WAS_TRIGGERED:
            ...
            sim.display_target()
            target_switch_timer = 0.0  
            total_sim_timer = 0.0 # 重置环境时，同时归零总计时器（如果需要的话。如果不想被重置，可以删掉这行）
